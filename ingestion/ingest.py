...
def get_processed_files_from_db(supabase: Client):
    ...
    try:
        response = (
            supabase.table(settings.DB_INGESTION_LOG_TABLE).select("file_path, checksum").execute()
        )
...
def main():
    ...
    current_files = {
        ...
        and (f.endswith(".pdf") or f.endswith(".md"))
    }
    ...
    files_to_update = {
        f for f in current_files if f in processed_log and current_files[f] != processed_log[f]
    }
    ...
    if files_requiring_deletion:
        ...
        for file_path in files_requiring_deletion:
            # Delete from Neo4j
            graph.query(
                "MATCH (s:Source {uri: $source_path})-[*0..]-(n) DETACH DELETE s, n",
                params={"source_path": file_path},
            )
            # Delete from Supabase Vector Store
            supabase.table(settings.DB_VECTOR_TABLE).delete().eq(
                "metadata->>source", file_path
            ).execute()
            # Delete from Ingestion Log
            supabase.table(settings.DB_INGESTION_LOG_TABLE).delete().eq(
                "file_path", file_path
            ).execute()
    ...
    for file_path in files_to_process:
        ...
        supabase.table(settings.DB_INGESTION_LOG_TABLE).upsert(
