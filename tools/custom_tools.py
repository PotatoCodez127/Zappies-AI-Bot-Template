...
# --- Slot Finding Helper (No changes needed here) ---
...
# --- Main Tool Function (No changes needed here) ---
def book_zappies_onboarding_call(full_name: str, email: str, company_name: str) -> str:
    """Finds the next available 60-minute slot and books a Google Calendar appointment."""
    try:
        logger.info("--- ACTION: Initiating Google Calendar Booking via Service Account ---")
...
# --- Tool Factory (No changes needed here) ---
def get_custom_tools() -> list:
    ...
    tools = [
        StructuredTool.from_function(
            ...
            description="Use this tool to find the next available slot and book a new onboarding call ONLY after you have collected the user's full name, email, and company name AND after the user has confirmed these details are correct."
        )
    ]
    return tools