...
# --- NEW CALLBACK HANDLER CLASS ---
class ToolCallbackHandler(BaseCallbackHandler):
    """Callback handler to store tool calls."""
    def __init__(self):
        self.tool_calls = []

    def on_agent_action(self, action, **kwargs: Any) -> Any:
        """Run when agent takes an action and append the tool name to our list."""
        if action.tool:
            self.tool_calls.append({"name": action.tool})
...
def create_agent_executor(memory, conversation_id: str):
    ...
    agent_executor = AgentExecutor(
        ...
        callbacks=[tool_callback] # <-- Pass the callback here
    )
