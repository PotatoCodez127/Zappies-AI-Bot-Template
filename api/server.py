...
app = FastAPI(
    title=settings.API_TITLE, description=settings.API_DESCRIPTION, version=settings.API_VERSION
)
...
class SupabaseChatMessageHistory(BaseChatMessageHistory):
    ...
    def messages(self):
        ...
        response = (
            ...
            .select("history")
            .eq("conversation_id", self.session_id)
            .execute()
        )
    ...
    def add_messages(self, messages: list[BaseMessage]) -> None:
        ...
        supabase.table(self.table_name).upsert(
            {"conversation_id": self.session_id, "history": updated_history, "status": "active"}
        ).execute()
    ...
    async with lock:
        try:
            ...
            if status_response.data and status_response.data[0].get("status") == "handover":
                logger.info(
                    f"Conversation {request.conversation_id} is in handover. Bypassing agent."
                )
        ...
        async with agent_semaphore:
            try:
                ...
                agent_executor, tool_callback = create_agent_executor(
                    memory, conversation_id=request.conversation_id
                )

                sast_tz = pytz.timezone("Africa/Johannesburg")
                current_time_sast = datetime.datetime.now(sast_tz).strftime(
                    "%A, %Y-%m-%d %H:%M:%S %Z"
                )
                ...
                if not agent_output or not agent_output.strip():
                    logger.warning(
                        f"Agent for convo ID {request.conversation_id} generated an empty response. Sending a default message."
                    )
                ...
                logger.error(
                    f"Error in /chat for conversation_id {request.conversation_id}: {e}",
                    exc_info=True,
                )
...
async def confirm_meeting(meeting_id: str):
    ...
    try:
        ...
        if not response.data:
            return (
                "<h1>Meeting Not Found</h1><p>This confirmation link is invalid or has expired.</p>"
            )
        ...
        supabase.table("meetings").update(
            {"status": "confirmed", "google_calendar_event_id": created_event.get("id")}
